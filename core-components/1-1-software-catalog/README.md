...
## System Model
See `system-model.md` for detailed view.
...
## YAML File Format
See `yaml-file-format.md` for detailed view.
...
- Each reference can be expressed in one of two ways:
1. compact string
2. compound reference structure
...
1. Optionally, the kind, followed by a colon
2. Optionally, the namespace, followed by a forward slash
3. The name
...
spec:
  ...
  owner: group:pet-managers   # <---- THIS IS A REFERENCE
  providesApis:
    - petstore   # <---- THIS IS A REFERENCE
    - internal/streetlights   # <---- THIS IS A REFERENCE
    - hello-world   # <---- THIS IS A REFERENCE
```
